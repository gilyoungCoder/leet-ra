...
시대인재 실제 제작 hwpx에서 `<hp:t>` 내부 텍스트만 비운 **빈 템플릿**을 베이스로 사용합니다 (표/2단/헤더/스타일 유지).

| 양식 | `--format` | 문제지 | 해설지 | 자연어 트리거 |
|------|-----------|-------|-------|-------------|
| 26추리 (기본) | `default` | `시대인재_문제지_빈.hwpx` (84KB) | `시대인재_해설지_빈.hwpx` (50KB) | "hwpx로", "시대인재 양식" |
| 전국 모의고사 | `jeonguk` | `전국_문제지_빈.hwpx` (366KB) | `전국서바_해설지_빈.hwpx` (61KB) | "전국 양식", "전국으로" |
| 서바이벌 | `serva` | `서바_문제지_빈.hwpx` (366KB) | `전국서바_해설지_빈.hwpx` (61KB) | "서바 양식", "서바로" |

- 교열 매뉴얼 자동 반영: **묶음 빈 칸(U+2060)** 문항번호↔발문, 선지기호↔내용, ㄱㄴㄷ↔내용
- 스타일 적용: `문제`, `선택지`, `보기박스`, `보기내용(내어쓰기)`, `지문 (테두리)`, `문항번호`, `정답_10pt`, `해시태그`, `정오판단_설명`
- `leet_ra/exporter/export_docx.py` — python-docx 2단 레이아웃 대체 출력
- 출력 디렉터리: `output/` (gitignore 대상)
- 빈 템플릿 재생성: `python3 leet_ra/exporter/build_blank_template.py`
