...
| | OMC 있음 | OMC 없음 |
|---|---------|---------|
| `/project:analyze` 슬래시 커맨드 | 동작 | 동작 |
| 자연어 "문항 만들어줘" → 에이전트 자동 트리거 | **동작** | 안 됨 |
| opus 서브에이전트 별도 spawn | **동작** | 같은 대화에서 처리 |
| review 독립 에이전트 분리 | **동작** | 같은 대화에서 처리 |
| 파이프라인 자동 순차 오케스트레이션 | **동작** | 수동으로 각 단계 실행 |

**결론: OMC를 설치해야 이 시스템의 진짜 성능이 나옵니다.**
